...
logging.basicConfig(format='%(asctime)s,%(msecs)03d %(levelname)-8s [%(filename)s:%(lineno)d] %(message)s',
                    datefmt='%Y-%m-%d:%H:%M:%S', level=logging.DEBUG, filename='app_log.log')
...
def isis_topology_trace(topo_dict: dict, source: str, dest: str, hop: int = 0) -> list:
    ...
    current_node = source
    previous_node = ''
    logging.debug(f'Starting ISIS topology trace')
    while True:
        if current_node == dest:
            ...
            if dest in topo_dict[current_node]['nhsystems']:
                ...
                logging.debug(f'Current node: {current_node}, Edge: {edge}')
                if hop > 0:
                    if previous_node != '':
                ...
                logging.debug(f'Current node: {current_node}, End: {dest}')
                logging.debug(f'{hop_dict}\n{end_dict}')
                hop_list.append(hop_dict)
                hop_list.append(end_dict)
                current_node = dest
            else:
                logging.error(f'Unable to trace - {dest} not part of the topology')
                break
    return hop_list
...
def first_hop(output: str) -> str:
    ...
    for lines in output.splitlines():
        no_newline_output = lines.split()
        logging.debug(f'FISRT HOP Parsed output: {no_newline_output}\n')
...
def single_device_collection(node_ip: str, show: str) -> str:
    ...
    if 'not_connected' in packed_output:
        logging.error(f'Unable to connect to device {node_ip}')
        exit(1)
    else:
...
def gather_topology(pe_dict: dict, topo_dict: dict, target_device: str = None) -> dict:
    ...
    logging.info(f"Gathering ISIS topology - if it's a full discovery could take some time")
        ...
        for device in output_isis:
            if device != 'not_connected':
                ...
                if device in pe_dict:
                    ...
                    logging.debug(f'"NOT_CONNECTED" OUTPUT COLLECTION\n{"-"*30}')
                    logging.debug(f'output {show_topo} for {device}:\n{"-" * 30}\n{single_ouptut}\n\n')
                    if device in single_ouptut:
                        topology_population(topo_dict, single_ouptut)
                    else:
                        logging.error(f'WARNING: Topology incomplete, unable to reach {device}')
    else:
        if target_device in pe_dict:
            lo10 = pe_dict[target_device]
            single_ouptut = single_device_collection(lo10, show_topo)
            logging.debug(f'TARGETED OUTPUT COLLECTION\n{"-" * 30}')
            logging.debug(f'output {show_topo} for {target_device}:\n{"-" * 30}\n{single_ouptut}\n\n')
            try:
                topology_population(topo_dict, single_ouptut)
            except TypeError:
                logging.error(f'WARNING: Topology incomplete, unable to reach {target_device}')
    return topo_dict


def run(start_node: str, ip: str, topo_dict: dict, soure_ip: str, vrf: str = None) -> tuple:
    ...
    if vrf is not None:
        ...
        show_cef_rpf = f'show cef vrf {vrf} {soure_ip} detail'
    else:
        show_cef = f'show cef {ip} detail'
        show_cef_rpf = f'show cef {soure_ip} detail'
    if 'CSR' in start_node:
        logging.info(f'Running trace from/to CSR')
        ...
        logging.info(f'Running trace from/to PE or vRouter')
...
def full_trace(topo_dict: dict, show_cef: str, start_node: str, isis_hop: str = None) -> tuple:
    ...
    end_node = ''
    end_ip = ''
    logging.info(f'Starting Full Trace - start node: {start_node}')
    tunn_num = ''
    while True:
        cef_output = single_device_collection(current_ip, show_cef)
        logging.debug(f'CEF OUTPUT - {current_node}\n{"-" * 30}\n{cef_output}\n\n')
        cef_hop_net, cef_int = cef_nhop(cef_output)
        if end_node != '':
            break
        else:
            if 'default' in cef_hop_net:
                logging.error(f'Unable to trace to specific network, cef next-hop towards default route')
                exit(1)
            elif 'null0' in cef_hop_net:
                logging.error(f'Null0 route in {current_node} for the traced IP. Please review the routing!')
                exit(1)
            else:
                if 'recursive' in cef_int:
                    if check_ip(cef_hop_net):
                        if 'CSR' in current_node:
                            ...
                            next_node = isis_src(current_ip, cef_hop_net)
                            next_ip = full_db[next_node]
                            logging.debug(f'ISIS Topology trace from {current_node} to {next_node}\n')
                            if len(hop_list) > 0:
                                current_hop = [x for x, y in hop_list[-1].items()][0] + 1
                        ...
                        logging.error(f'Unable to trace cef next_hop {cef_hop_net}, cef interface {cef_int}')
                        exit(1)
                    ...
                    logging.debug(f'---- NHOP Tunnel detected ---\nOutput\n{"-"*30}\n{tunnel_descrip}\n'
                    ...
                    try:
                        ...
                        logging.error(f'Device {next_node} not found in JSON database')
                        exit(1)
                elif re.search(r'TenGi|Giga|BV', cef_hop_net):
                    end_ip = current_ip
                    end_node = current_node
...
def printing_trace(hop_list: list) -> None:
    print(f'{"Hop":<5} {"Ingress IF":<20} {"Node":<15} {"Egress IF":<5}')
    print(f'{"-" * 3:<5} {"-" * 10:<5} {"-" * 25:<5} {"-" * 10:<5}')
    for hops in hop_list:
        for hop in hops:
            print(f'{hop:<5} {hops[hop]["ingress_if"]:<10} {hops[hop]["node"]:>10} {hops[hop]["egress_if"]:<10}')
    print(f'\n')
...
def get_conn_int(node: str, vrf: str, full_db: dict):
    ...
    logging.debug(f'GET CONNECTED IF:\n{"-"*30}\n{show_route}')
    for line in show_route.splitlines():
        if re.search(Regex.connected_route, line, re.M):
            ...
            if 'Loopback' not in interface:
                ...
                break
    logging.debug(f'GET CONNECTED IF - PARSED: Interface {interface} - IP {temp_ip}')
    return temp_ip
...
if __name__ == '__main__':
    ...
    with open('topo.json', 'r') as file_two:
        ...
        file.close()
    source = 'SLSLC00102A-CS000-CSR001'
    vrf = 'SERVICE-MGMT'
    ip = '10.233.49.4'
    logging.warning('Running check on conneted interfaces')
    source_ip = get_conn_int(source, vrf, full_db)
    logging.warning('Start Trace')
    result, rfp_result = run(source, ip, topo_dict, source_ip, vrf)
    logging.warning('Trace finished')
    end_host, end_ip, hop_list = result
    rpf_host, rpf_ip, rpf_list = rfp_result
    printing_trace(hop_list)
    printing_trace(rpf_list)
    print(f'Finished execution. Took {(time.time() - start_time):.2f}')
